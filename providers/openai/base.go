...
func (p *OpenAIProvider) mergeCustomParams(requestMap map[string]interface{}, customParams map[string]interface{}) map[string]interface{} {
	...
	overwrite := base.GetCustomParamsOverwrite(customParams)
	mappedModel, _ := requestMap["model"].(string)
	selected := base.SelectCustomParamsForPhase(customParams, base.CustomParamsPhaseSend, p.GetOriginalModel(), mappedModel)
	return base.MergeSelectedCustomParams(requestMap, selected, overwrite)
}
