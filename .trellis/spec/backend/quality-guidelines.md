...
- `AllowExtraBody=true` 且 `relayMode` 为 `config.RelayModeResponses` 或 `config.RelayModeResponsesCompact`：以上游原始 JSON object 为请求体基础。
- Responses raw passthrough 只从结构体结果覆盖 `model`，确保上游收到映射后的模型名。
- 上述 raw passthrough contract 同时适用于 `/v1/responses` 与 `/v1/responses/compact`；compact 不能退回通用 extra body merge，否则未建模字段会被结构体重组。
...
- provider 测试断言相同行为对 compact endpoint 同样成立。
...
## Provider 请求构建契约：Responses Compact endpoint routing

### 1. Scope / Trigger

- Trigger：修改 `/v1/responses*` 路由、relay 分发、provider URL 映射或 Responses interface 签名时，必须检查 compact contract。
- 这是一个跨层 contract：Gin 路由、relay 意图判定、provider config、unsupported 行为和测试必须同步更新。

### 2. Signatures

- `POST /v1/responses`
- `POST /v1/responses/compact`
- `relay.NewRelayResponses(c *gin.Context)`
- `providers/base.ResponsesInterface`
  - `CreateResponses(relayMode int, request *types.OpenAIResponsesRequest)`
  - `CreateResponsesStream(relayMode int, request *types.OpenAIResponsesRequest)`
- `providers/base.ProviderConfig`
  - `Responses string`
  - `ResponsesCompact string`

### 3. Contracts

- `/v1/responses/compact` 不是 `/v1/responses` 的参数分支，provider URL 必须使用独立 `ResponsesCompact` 映射。
- relay 必须根据 path 识别 create/compact，并把 `relayMode` 传到 provider，而不是让 provider 猜测当前意图。
- compact 只有原生 OpenAI 渠道可用；`ChannelTypeOpenAI` 之外的 Azure、Azure V1、自定义 OpenAI-compatible 与其它非 OpenAI 渠道都必须视为 unsupported。
- 只有原生 OpenAI 默认提供 `ResponsesCompact` 映射：`/v1/responses/compact`。
- 自定义 OpenAI-compatible 渠道不能暴露 compact URI 配置入口；即使历史配置残留 `plugin.customize["17"]`，运行时也必须忽略并返回 unsupported。
- compact 请求保持 Responses 原始 request/response shape，成功响应允许 `object == "response.compaction"`，`output[].type == "compaction"`，`encrypted_content` 原样透传。
- compact 请求命中“当前渠道不支持 compact”时，只在现有重试机制允许时进入换渠道流程；候选集合只允许切到其它可用的原生 OpenAI 渠道，不要绕过全局 `RetryTimes` 语义单独为 compact 注入额外重试预算。

### 4. Validation & Error Matrix

- path 为 `/v1/responses/compact` 且 `stream=true` -> `400 invalid_request`
- compact 请求命中非原生 OpenAI 渠道、`CompatibleResponse=true`、不支持 Responses 的 provider 或缺少 compact URI -> `501 unsupported_api`
- compact 请求命中上游 `404`，或 `400` 且消息明确指向 `/responses/compact` 的 invalid URL / unsupported -> 归一化成可重试的 compact unsupported
- create 请求命中不支持 Responses 的 provider -> 继续走现有 chat compatibility fallback
- compact 请求命中支持 provider -> 只能走 Responses 原生路径，不能降级到 chat/completions

### 5. Good/Base/Bad Cases

- Good：`POST /v1/responses/compact` 被转发到上游 `/v1/responses/compact`，返回 `response.compaction` 和 `encrypted_content`。
- Good：首个渠道返回 compact unsupported 且 `RetryTimes > 0` 时，relay 会继续尝试其它候选渠道；当所有候选渠道都不支持时，最终仍返回同一条 deterministic compact unsupported 错误。
- Base：`POST /v1/responses` 继续走原有 `/v1/responses`，Responses custom params / extra body / usage 逻辑不回退。
- Bad：只新增 Gin 路由，但 provider 仍把 compact 请求发到 `/v1/responses`，或者把 compact 错误地降级到 chat compatibility。

### 6. Tests Required

- router 测试断言 `POST /v1/responses/compact` 已注册，且既有 `POST /v1/responses` 仍在。
- provider 测试断言 compact 会命中独立上游路径，并能承载 `response.compaction` / `encrypted_content`。
- provider 测试断言 Azure、Azure V1、自定义 OpenAI-compatible compact 都返回确定性的 `unsupported_api`。
- relay 测试断言 compact `stream=true` 直接拒绝。
- relay 测试断言 compact 不会回退到 chat compatibility。
- relay 测试断言 compact 对非 OpenAI 渠道不会尝试调用 provider 的 compact 上游路径。
- relay 测试断言 compact unsupported 会遵守现有 `RetryTimes` 语义，不会为 compact 单独突破 `RetryTimes=0`。
- relay 测试断言 compact 上游 `404` / `400 Invalid URL` 会归一化成可重试 unsupported，而普通 `/v1/responses` 不会误命中该归一化。
...
if strings.HasPrefix(path, "/v1/responses") {
	return provider.CreateResponses(config.RelayModeResponses, request)
}
```

这会把 compact 和 create 都压到同一个上游路径。

#### Correct

```go
relayMode := getResponsesRelayMode(path)
return provider.CreateResponses(relayMode, request)
```

先在 relay 判定 create/compact 意图，再把独立 relay mode 传给 provider URL 选择层。
