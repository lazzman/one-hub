		...
		for {
			...
			case err := <-errChan:
				if !errors.Is(err, io.EOF) {
					...
					responseBuilder.WriteString(errMsg)
					...
					if finalErr == nil && endHandler != nil {
						...
						if streamData != "" {
							formattedData := "data: " + streamData + "\n\n"
							select {
							case <-c.Request.Context().Done():
								// 客户端已断开，不执行任何操作，直接跳过
							default:
								// 客户端正常，发送数据
								c.Writer.Write([]byte(formattedData))
								c.Writer.Flush()
							}
							// 将 endHandler 补发的数据追加到 responseBuilder，确保日志保留完整输出原文
							responseBuilder.WriteString(formattedData)
						}
					}
		...
		for {
			...
			case err := <-errChan:
				if !errors.Is(err, io.EOF) {
					...
					responseBuilder.WriteString(err.Error())
...
func responseCustom(c *gin.Context, response *types.AudioResponseWrapper) *types.OpenAIErrorWithStatusCode {
	// 存储完整响应到上下文（原始响应体）
	hijack.StoreFullResponse(c.Request.Context(), hijack.ResponseTypeCustom, response.Body)
...
func mergeCustomParamsForPreMapping(requestMap map[string]interface{}, customParams map[string]interface{}) map[string]interface{} {
	overwrite := providersBase.GetCustomParamsOverwrite(customParams)
	originalModel, _ := requestMap["model"].(string)
	selected := providersBase.SelectCustomParamsForPhase(customParams, providersBase.CustomParamsPhasePreAdd, originalModel, "")
	return providersBase.MergeSelectedCustomParams(requestMap, selected, overwrite)
}
