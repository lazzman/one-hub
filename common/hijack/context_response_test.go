...
func TestAppendResponseToLogContent_StoresRawStreamBody(t *testing.T) {
	...
	if payload.Source.APIProvider != "openrouter" {
		t.Fatalf("unexpected api provider: %s", payload.Source.APIProvider)
	}
	if payload.Source.APISchema != "openai-compatible" {
		t.Fatalf("unexpected api schema: %s", payload.Source.APISchema)
	}
	if payload.Source.APIEndpoint != "chat-completions" {
		t.Fatalf("unexpected api endpoint: %s", payload.Source.APIEndpoint)
	}
	if payload.Source.APITransport != "stream" {
		t.Fatalf("unexpected api transport: %s", payload.Source.APITransport)
	}
...
func TestAppendResponseToLogContent_StoresCompleteRequestAndResponse(t *testing.T) {
	...
	if payload.Source.APISchema != "openai-responses" {
		t.Fatalf("unexpected api schema: %s", payload.Source.APISchema)
	}
}

func TestBuildLogPreviewPayload_PreservesOpenAICompatibleSchema(t *testing.T) {
	payload := BuildLogPreviewPayload(LogPreviewSource{
		ProtocolHint: "openai-compatible",
		Provider:     "openrouter",
		EndpointPath: "/v1/chat/completions",
	}, `{}`, nil)

	if payload.Source.ProtocolHint != "openai-compatible" {
		t.Fatalf("unexpected protocol hint: %s", payload.Source.ProtocolHint)
	}
	if payload.Source.APISchema != "openai-compatible" {
		t.Fatalf("unexpected api schema: %s", payload.Source.APISchema)
...
func TestBuildLogPreviewPayload_PreservesExplicitAPISourceFields(t *testing.T) {
	payload := BuildLogPreviewPayload(LogPreviewSource{
		ProtocolHint: "claude",
		Provider:     "anthropic",
		EndpointPath: "/claude/v1/messages",
		APIProvider:  "vertex-ai",
		APISchema:    "claude-messages",
		APIEndpoint:  "vertex-claude-messages",
		APITransport: "batch",
	}, `{}`, nil)

	if payload.Source.APIProvider != "vertex-ai" {
		t.Fatalf("unexpected api provider: %s", payload.Source.APIProvider)
	}
	if payload.Source.APISchema != "claude-messages" {
		t.Fatalf("unexpected api schema: %s", payload.Source.APISchema)
	}
	if payload.Source.APIEndpoint != "vertex-claude-messages" {
		t.Fatalf("unexpected api endpoint: %s", payload.Source.APIEndpoint)
	}
	if payload.Source.APITransport != "batch" {
		t.Fatalf("unexpected api transport: %s", payload.Source.APITransport)
	}
}
