...
func AppendResponseToLogContent(ctx context.Context) string {
	...
	responseData, _ := GetAndDeleteFullResponse(ctx)
	source, _ := GetAndDeleteLogPreviewSource(ctx)

	payload := BuildLogPreviewPayload(source, requestBody, responseData)
	return MarshalLogPreviewPayload(payload)
}
