...
import { getToolDisplayName } from '../toolDefinitionModel';
...
const UNIFIED_COLLECTION_KEYS = ['events', 'messages', 'toolCalls', 'reasoning', 'media'];

const isObjectLike = (value) => value !== null && typeof value === 'object' && !Array.isArray(value);

const toSerializableText = (value) => {
...
const extractUnifiedViewModel = (data) => {
  if (!isObjectLike(data)) return null;

  const viewModel = isObjectLike(data.viewModel) ? data.viewModel : data;
  const hasUnifiedCollections = UNIFIED_COLLECTION_KEYS.some((key) => Array.isArray(viewModel[key]));
  if (!hasUnifiedCollections) return null;

  return {
    ...viewModel,
    protocol: typeof viewModel.protocol === 'string' ? viewModel.protocol : 'unknown'
  };
};
  ...
  if (Array.isArray(message.content)) {
    return message.content
      .map((item) => {
        ...
        if (!isObjectLike(item)) return String(item ?? '');
        if (typeof item.text === 'string') return item.text;
        if (typeof item.thinking === 'string') return item.thinking;
        if (typeof item.content === 'string') return item.content;
        return toSerializableText(item);
      })
      .filter(Boolean)
      .join('\n');
  }
...
const buildToolPreviewFallbackKey = ({ toolCall, index, argumentsText }) => {
  const name = normalizeString(toolCall?.name) || 'unknown';
  const normalizedArguments = normalizeString(argumentsText);

  if (normalizedArguments) {
    return `fallback:${name}:arguments:${normalizedArguments}`;
  }

  const path = normalizeString(toolCall?.path);
  if (path) {
    return `fallback:${name}:path:${path}`;
  }

  return `fallback:${name}:index:${index}`;
};

const buildToolPreviewKey = ({ toolCall, index, argumentsText }) => {
  const callId = normalizeString(toolCall?.id || toolCall?.call_id || toolCall?.toolCallId || toolCall?.tool_use_id || toolCall?.toolUseId);
  if (callId) {
    return {
      key: `call_id:${callId}`,
      callId
  ...
  return {
    key: buildToolPreviewFallbackKey({ toolCall, index, argumentsText }),
    callId: ''
  };
};

const mergeToolPreviewCallRecord = ({ existing, incoming }) => {
  if (!existing) return incoming;

  return {
    callId: incoming.callId || existing.callId,
    name: incoming.name || existing.name,
    argumentsText: incoming.argumentsText || existing.argumentsText,
    hasResult: existing.hasResult || incoming.hasResult,
    resultValue: incoming.hasResult ? incoming.resultValue : existing.resultValue,
    callPath: incoming.callPath || existing.callPath,
    resultPath: incoming.hasResult ? incoming.resultPath : existing.resultPath
  };
};

/**
 * 检测统一解析模型（parseContent 输出）的内容
 * @param {any} data - 统一模型对象或含 viewModel 的对象
 * @param {string} path - 基础路径
 * @returns {Array} 检测结果数组
 */
export function detectUnifiedModelContent(data, path = '') {
  const viewModel = extractUnifiedViewModel(data);
  if (!viewModel) return [];

  const pathPrefix = path ? `${path}.` : '';
  const protocol = viewModel.protocol || 'unknown';
  const results = [];

  // 1) 消息片段（文本/Markdown）
  const messages = Array.isArray(viewModel.messages) ? viewModel.messages : [];
  messages.forEach((message, index) => {
    const text = extractMessageText(message).trim();
    if (!text) return;

    const detectedType = detectContentTypeForAIResponse(text);
    results.push({
      type: detectedType === ContentType.PLAIN_TEXT ? ContentType.MARKDOWN : detectedType,
      content: text,
      role: message?.role || 'unknown',
      protocol,
      path: `${pathPrefix}messages[${index}].text`
    });
  });

  // 2) 工具调用与工具结果（按 call_id/兜底键去重）
  const toolCalls = Array.isArray(viewModel.toolCalls) ? viewModel.toolCalls : [];
  const dedupedToolCallMap = new Map();

  toolCalls.forEach((toolCall, index) => {
    if (!isObjectLike(toolCall)) return;

    const name = normalizeString(toolCall.name);
    const argumentsValue = toolCall.arguments ?? toolCall.args ?? toolCall.input ?? '';
    const argumentsText = typeof argumentsValue === 'string' ? argumentsValue : toSerializableText(argumentsValue);
    const resultValue = toolCall.result ?? toolCall.output ?? toolCall.response;
    const hasResult = resultValue !== undefined && resultValue !== null && toSerializableText(resultValue).trim() !== '';

    const { key, callId } = buildToolPreviewKey({ toolCall, index, argumentsText });
    ...
    const incomingRecord = {
      ...
      hasResult,
      resultValue,
      callPath: `${pathPrefix}toolCalls[${index}]`,
      resultPath: `${pathPrefix}toolCalls[${index}].result`
    };

    const mergedRecord = mergeToolPreviewCallRecord({
      existing: dedupedToolCallMap.get(key),
      incoming: incomingRecord
    });

    dedupedToolCallMap.set(key, mergedRecord);
  });

  Array.from(dedupedToolCallMap.values()).forEach((toolCallRecord) => {
    if (toolCallRecord.callId || toolCallRecord.name || normalizeString(toolCallRecord.argumentsText)) {
      results.push({
        type: ContentType.TOOL_CALL,
        id: toolCallRecord.callId,
        name: toolCallRecord.name,
        arguments: toolCallRecord.argumentsText,
        protocol,
        path: toolCallRecord.callPath
      });
    }

    if (toolCallRecord.hasResult) {
      results.push({
        type: ContentType.TOOL_RESULT,
        toolCallId: toolCallRecord.callId,
        name: toolCallRecord.name,
        content: toolCallRecord.resultValue,
        protocol,
        path: toolCallRecord.resultPath
      });
    }
  });

  // 3) 推理片段
  const reasoningList = Array.isArray(viewModel.reasoning) ? viewModel.reasoning : [];
  reasoningList.forEach((reasoning, index) => {
    const text =
      typeof reasoning === 'string'
        ...
        : typeof reasoning?.text === 'string'
          ...
          : typeof reasoning?.summary === 'string'
            ? reasoning.summary
            : toSerializableText(reasoning?.raw ?? reasoning);
    ...
    results.push({
      ...
      content: text,
      reasoningType: reasoning?.type || 'reasoning',
      protocol,
      path: `${pathPrefix}reasoning[${index}]`
    });
  });

  // 4) 多模态片段
  const mediaList = Array.isArray(viewModel.media) ? viewModel.media : [];
  mediaList.forEach((media, index) => {
    if (!isObjectLike(media)) return;

    const mediaType = String(media.type || '').toLowerCase();
    const mimeType = media.mimeType || media.mime_type || '';
    const url = media.url || media.fileUri || media.file_uri || '';
    const rawData = media.data || media.base64 || '';
    const dataUri =
      typeof rawData === 'string' && rawData
        ? rawData.startsWith('data:')
          ? rawData
          : `data:${mimeType || 'application/octet-stream'};base64,${rawData}`
        : '';

    if (mediaType === 'image') {
      if (url) {
        results.push({
          type: isBase64Image(url) ? ContentType.IMAGE_BASE64 : ContentType.IMAGE_URL,
          content: url,
          mimeType,
          protocol,
          path: `${pathPrefix}media[${index}].url`
        });
      } else if (dataUri) {
        results.push({
          ...
          mimeType,
          protocol,
          path: `${pathPrefix}media[${index}].data`
        });
      }
      return;
    }
    ...
    if (mediaType === 'audio') {
      const audioContent = rawData || url;
      if (audioContent) {
        results.push({
          type: ContentType.AUDIO,
          content: audioContent,
          mimeType,
          isUrl: Boolean(url && !rawData),
          protocol,
          path: `${pathPrefix}media[${index}]`
        });
      }
      return;
    }
    ...
    if (mediaType === 'file' || mediaType === 'document') {
      results.push({
        type: mediaType === 'document' ? ContentType.DOCUMENT : ContentType.FILE,
        content: url || rawData,
        mimeType,
        source: media.raw || media,
        protocol,
        path: `${pathPrefix}media[${index}]`
      });
      return;
  ...
  // 协议未知或无可提取项时，降级为原文片段
  if (results.length === 0 || protocol === 'unknown') {
    const fallbackCandidates = [viewModel?.response?.raw, viewModel?.request?.raw, viewModel?.response?.parsed, viewModel?.request?.parsed]
      .map((item) => toSerializableText(item).trim())
      .filter(Boolean);
...
function isClaudeResponse(data) {
  return data && typeof data === 'object' && data.type === 'message' && data.role === 'assistant' && Array.isArray(data.content);
}
...
export function detectContentTypes(data, path = '') {
  ...
  if (typeof data === 'string') {
    ...
    try {
      ...
      // 优先消费统一模型（parseContent 结果）
      const unifiedViewModel = extractUnifiedViewModel(parsed);
      if (unifiedViewModel) {
        return detectUnifiedModelContent(parsed, path);
      }
  ...
  // 优先消费统一模型（parseContent 结果）
  const unifiedViewModel = extractUnifiedViewModel(data);
  if (unifiedViewModel) {
    return detectUnifiedModelContent(data, path);
  }
    ...
    for (const message of requestBody.messages) {
      ...
      if (Array.isArray(message.content)) {
        for (const item of message.content) {
          ...
          if (item.type === 'tool_use' || item.type === 'tool_result' || item.type === 'image' || item.type === 'document') {
            return RequestFormat.CLAUDE;
          }
...
export function detectOpenAIRequestContent(requestBody) {
  ...
  if (Array.isArray(requestBody.tools) && requestBody.tools.length > 0) {
    results.push({
      ...
      tools: requestBody.tools.map((tool, index) => getToolDisplayName(tool, index, 'unknown')),
      rawTools: requestBody.tools,
      path: 'tools'
    });
  }
    ...
    requestBody.tools.forEach((toolGroup) => {
      if (Array.isArray(toolGroup.functionDeclarations)) {
        toolGroup.functionDeclarations.forEach((func) => {
          toolNames.push(getToolDisplayName(func, toolCount, 'unknown'));
          toolCount++;
        });
      }
    });
  ...
  if (message.role !== undefined) {
    ...
    if (Array.isArray(message.content)) {
      ...
      for (const item of message.content) {
        ...
        // OpenAI 特有类型
        if (item.type === 'image_url' || item.type === 'input_audio' || item.type === 'input_image') {
          return MessageFormat.OPENAI;
        }
        // Claude 的 text 块可能有 cache_control
        if (item.type === 'text' && item.cache_control !== undefined) {
    ...
    if (typeof message.content === 'string') {
      ...
      message.content.forEach((item, index) => {
        ...
        if (typeof item === 'string') {
          ...
          // 文本内容块
          // OpenAI / Responses 格式：text / input_text / output_text 都直接识别为 Markdown
          ...
          // Responses input_image
            ...
            results.push({
              type: isBase64Image(item.image_url) ? ContentType.IMAGE_BASE64 : ContentType.IMAGE_URL,
              content: item.image_url,
              detail: item.detail,
              path: `${itemPath}.image_url`
            });
          }
          // 音频输入
...
export function hasToolCall(message) {
  ...
  switch (format) {
    ...
    case MessageFormat.CLAUDE:
      return Array.isArray(message.content) && message.content.some((c) => c.type === 'tool_use');
    case MessageFormat.GEMINI:
      return Array.isArray(message.parts) && message.parts.some((p) => p.functionCall !== undefined);
    default:
      return false;
  }
}
...
export function isToolResult(message) {
  ...
  switch (format) {
    ...
    case MessageFormat.CLAUDE:
      return Array.isArray(message.content) && message.content.some((c) => c.type === 'tool_result');
    case MessageFormat.GEMINI:
      return Array.isArray(message.parts) && message.parts.some((p) => p.functionResponse !== undefined);
    default:
      return false;
  }
}
...
export function hasMultimediaContent(message) {
  ...
  switch (format) {
    case MessageFormat.OPENAI:
      if (Array.isArray(message.content)) {
        return message.content.some((c) => c.type === 'image_url' || c.type === 'input_audio');
      }
      return false;
    case MessageFormat.CLAUDE:
      if (Array.isArray(message.content)) {
        return message.content.some((c) => c.type === 'image' || c.type === 'document');
      }
      return false;
    case MessageFormat.GEMINI:
      if (Array.isArray(message.parts)) {
        return message.parts.some((p) => p.inlineData !== undefined || p.fileData !== undefined);
      }
      return false;
    default:
      return false;
