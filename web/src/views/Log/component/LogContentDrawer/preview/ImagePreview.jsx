 ...
 */
const ImagePreview = ({ images, open, onClose, initialIndex = 0 }) => {
  ...
  useEffect(() => {
    ...
    setCurrentIndex(Math.min(Math.max(safeIndex, 0), Math.max((images?.length || 1) - 1, 0)));
    resetState();
  ...
  const handleMouseDown = (e) => {
    if (e.button !== 0 || e.target !== imageRef.current) {
      return;
    }
