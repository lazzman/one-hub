...
import { Accordion, AccordionDetails, AccordionSummary, Box, Chip, Typography, Alert } from '@mui/material';
...
import { buildNonConversationResponse, buildToolCallSummary } from './parsers/auditModel';
...
const ResponseSection = React.forwardRef(
  ({ protocol, viewModel, response, rawResponseBody, finalAssistantText, toolCalls, reasoning, media, linearTrace, sectionRefs }, ref) => {
    ...
    const toolCallSummary = useMemo(() => buildToolCallSummary(normalizedToolCalls), [normalizedToolCalls]);
    ...
    const nonConversationResponse = useMemo(
      () =>
        buildNonConversationResponse(responsePayload, viewModel?.source?.display?.schema?.key || viewModel?.source?.apiSchema || protocol),
      [responsePayload, viewModel, protocol]
    );
      ...
      <Box id="response" ref={ref}>
        ...
        {capabilities.finalAnswer && finalResponseText && (
          <Box id="response-final-answer" ref={(el) => sectionRefs && (sectionRefs.current['response-final-answer'] = el)} sx={{ mb: 2 }}>
            <SectionTitle icon={<AutoAwesomeIcon />} title="最终回复" color="primary.main" />
            <CopyablePanel
              title="最终回复内容"
        ...
        {capabilities.reasoning && (
          <Box id="response-reasoning" ref={(el) => sectionRefs && (sectionRefs.current['response-reasoning'] = el)} sx={{ mb: 2 }}>
            <SectionTitle icon={<AutoAwesomeIcon />} title="推理过程" color="secondary.main" />
            {reasoningText ? (
              <CopyablePanel
                title="推理内容"
          ...
          <Box id="response-tool-calls" ref={(el) => sectionRefs && (sectionRefs.current['response-tool-calls'] = el)} sx={{ mb: 2 }}>
            ...
            {toolCallSummary.length > 0 && (
              <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1, mb: 1 }}>
                {toolCallSummary.map((item) => (
                  <Chip
                    key={item.name}
                    label={`${item.name} ×${item.count}${item.results ? ` / 结果 ${item.results}` : ''}`}
                    color={item.results ? 'success' : 'warning'}
                    variant="outlined"
                    size="small"
                  />
                ))}
        ...
        {capabilities.trace && (
          <Box id="response-linear-trace" ref={(el) => sectionRefs && (sectionRefs.current['response-linear-trace'] = el)} sx={{ mb: 2 }}>
            <SectionTitle icon={<TimelineIcon />} title="线性调用轨迹" color="warning.main" />
            <LinearTraceSection linearTrace={normalizedLinearTrace} />
          </Box>
        )}

        {nonConversationResponse?.items?.length > 0 && (
          <Box sx={{ mb: 2 }}>
            <SectionTitle
              icon={<ArticleIcon />}
              title={`${NON_CONVERSATION_KIND_LABELS[nonConversationResponse.kind] || '通用'}响应摘要`}
              color="primary.main"
            />
            <CopyablePanel
              title="响应摘要"
              content={nonConversationResponse.items}
              rawContent={nonConversationResponse.items}
              visibleChipTypes={nonConversationResponse.kind === 'images' ? ['image_url', 'image_base64'] : []}
              sx={panelSx}
            />
          </Box>
        )}
        ...
        {capabilities.rawResponse && responseRawText && (
          <Box id="response-raw" ref={(el) => sectionRefs && (sectionRefs.current['response-raw'] = el)} sx={{ mb: 2 }}>
            <SectionTitle icon={<ArticleIcon />} title="原始响应" color="text.primary" />
            <Accordion defaultExpanded>
              <AccordionSummary expandIcon={<ExpandMoreIcon />}>
                <Typography fontWeight={700}>原始响应</Typography>
              </AccordionSummary>
              <AccordionDetails>
                <CopyablePanel
                  title="响应原文"
