const defaultConfig = {
  ...
  prompt: {
    ...
    custom_parameter:
      '支持通过 JSON 注入额外参数（可嵌套）。可用控制项：overwrite：设为 true 覆盖同名字段，未设置或 false 时仅补充缺失字段；per_model：设为 true 后按模型名进行参数覆盖；pre_add：设为 true 时在支持前置 JSON 注入的接口入口按原始请求模型名完成参数覆盖，否则会在发送请求前按映射后的模型名进行参数覆盖。对于非 JSON 请求或特殊协议，是否支持取决于渠道实现能力。',
